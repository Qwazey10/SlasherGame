...
// Called every frame
void UAC_AbilityComponent::TickComponent(float DeltaTime, ELevelTick TickType,
...
void UAC_AbilityComponent::Grant_Experience_AbilityID_001(float AmtOfEXPToAdd)
{
	...
	if (LocalOwner->ActorHasTag("Enemy"))
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("AC_AbilityComponent -- Ability ID 001 Called on an Enemy %s -- No EXP to ADD"),
		       *GetOwner()->GetName());
	}
}
