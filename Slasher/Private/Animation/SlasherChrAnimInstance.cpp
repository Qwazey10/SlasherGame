	...
	if (OwningActor != nullptr)
	{
		if (OwningActor->ActorHasTag(FName("Player")))
		{
			...
			if (PlayerCharacter != nullptr)
			{
				...
				bChrAnimInst_IsJumping = PlayerCharacter->GetCharacterMovement()->IsFalling();// Set IsSprinting from Pawn.
				...
				if (bChrAnimInst_Debug)
				{
					...
					if (GEngine)
					{
						...
						FString TestString = FString::Printf(TEXT("Character Speed From Anim Graph -- %f"), ChrAnimInst_Speed);
						FString TestString2 = FString::Printf(TEXT ("Anim Bool IsMovingBoolean -- %d"), bChrAnimInst_IsMoving);
						FString TestString3 = FString::Printf(TEXT ("Anim Bool IsSprinting? -- %d"), bChrAnimInst_IsSprinting);
						FString TestString4 = FString::Printf(TEXT ("Anim Bool IsFalling? -- %d"), bChrAnimInst_IsJumping);
