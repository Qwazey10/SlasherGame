	...
	if (InVisibility == ESlateVisibility::Visible)
	{
		...
		if (SlasherGI)
		{
			for (int i = 0; i < SlasherGI->Inventory.Num(); i++)
			{
				...
				if (bFoundItemSlot)
				{
					...
					FItemStruct* FoundItemStruct = SlasherGI->ItemDataTable->FindRow<FItemStruct>(
						RowName, "Get Row if Inventory Has Item");
				...
				{
					UE_LOG(LogTemp, Display, TEXT("UseItemContextWidget.cpp -- HandleVisibilityChanged() -- For Loop -- NotEmptySlot Slot Found at Index: %d"), i);
				}
			}
		}
		else
		{
			GEngine->AddOnScreenDebugMessage(-1, 20, FColor::Red,
			                                 "UseItemContextWidget.cpp - On Vis Change -- GameInstance is NOT VALID");
		}
	}
